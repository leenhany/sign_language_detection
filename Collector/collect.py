...
# TODO: Display landmark movement dictionary
...
DATA_CSV_PATH = 'Collector/data.csv'
LABEL_PATH = 'Collector/label.csv'
MOVEMENT_CSV_PATH = 'Collector/m_data.csv'

TRAINER_DATA_DIR = 'Trainer/model/data/'
...
# Listener forword, mode, record
# TODO: Movement expansion add new mode
...
def calc_landmark_list(image, landmarks, handedness):
	...
	# Retrieve landmark coordinates within camera frame
	for _, landmark in enumerate(landmarks.landmark):
		landmark_x = min(int(landmark.x * image_width), image_width - 1)
		landmark_y = min(int(landmark.y * image_height), image_height - 1)
		landmark_z = min(int(landmark.z * image_width), image_width - 1)
...
# Movement indicator reference display
...
def main():
	...
	while True:
		...
		if key == 27:  # ESC
			...
			try:
				...
				print("Data not copied to other modules. Copy data files if needed.")
				pass
		...
		if results.multi_hand_landmarks is not None:
			for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
				...
				# Record landmarks/movement
				if(mode == 1):
					
					# Static sign indices (not J and Z)
					...
					# J with m_label_index = 1
						...
						# Track/Display pinky finger tip
						track_landmark_movement_index = 20
						...
						# Record landmarks position/sequences
						if(trigger == 1):
					...
					# Z with m_label_index 2
						...
						# Track/Display index finger tip
						track_landmark_movement_index = 8
						LANDMARK_MOVEMENT_SEQUENCE.append(landmark_list[track_landmark_movement_index])
					...
					# NONE(movement noise) with m_label_index 0
					elif(current_index == 26):
						
						#TODO: Track custom index landmark, (use a trackbar?)
						track_landmark_movement_index = 0
						LANDMARK_MOVEMENT_SEQUENCE.append(landmark_list[track_landmark_movement_index])
	...
	#TODO: Write collected data to database
